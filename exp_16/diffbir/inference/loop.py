...
class InferenceLoop:
    ...
    def load_lq(self) -> Generator[Image.Image, None, None]:
        ...
        for file_name in sorted(os.listdir(self.args.input)):
            ...
            # 【读取对应的rgb图像】
            rgb_file_path = os.path.join(self.args.rgb_path, file_name)
            if os.path.exists(rgb_file_path):
                rgb_image = Image.open(rgb_file_path).convert("RGB")
                print(f"load rgb: {rgb_file_path}")
            else:
                print(f"RGB file {rgb_file_path} not found. Using None as RGB image.")
                rgb_image = None
            ...
            for i in range(num_batches):
                ...
                with torch.autocast(self.args.device, auto_cast_type):
                    batch_samples = self.pipeline.run(
                        ...
                        self.args.order,
                        # condition,      # 【读取对应的特征】
                        np.tile(rgb[None], (n_inputs, 1, 1, 1)),
