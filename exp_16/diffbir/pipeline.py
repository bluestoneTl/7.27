...
class Pipeline:
    ...
    def apply_cldm(
        ...
        if not vae_encoder_tiled and not cldm_tiled:
            ...
            rgb = pad_to_multiples_of(rgb, multiple=64)
        else:
            cond_img = pad_to_multiples_of(cond_img, multiple=8)
            rgb = pad_to_multiples_of(rgb, multiple=8)
    ...
    def run(
        ...
        rgb_tensor = (
            torch.tensor(rgb, dtype=torch.float32, device=self.device)
            .div(255)
            .clamp(0, 1)
            .permute(0, 3, 1, 2)
            .contiguous()
        ...
        sample = self.apply_cldm(
            ...
            rgb_tensor,  # 传递 rgb 给 apply_cldm 方法
            train_stage,
        )
