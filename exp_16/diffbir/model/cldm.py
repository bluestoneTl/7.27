...
class ControlLDM(nn.Module):
    ...
    def forward(self, x_noisy, t, cond, is_first_stage):
        ...
        if c_rgb.shape[2] != 64:
            c_rgb = F.interpolate(c_rgb, size=64, mode='bilinear', align_corners=True)
        control = self.controlnet_LCA(x=x_noisy, hint=z_hint, rgb=c_rgb, timesteps=t, context=c_txt, unet_encoder_results=hs)  # LCA加入RGB图像，用交叉注意力融合
